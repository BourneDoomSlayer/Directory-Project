	...
	public static class DoublyLinkedList<E> implements List<E> {
		...
		public void add(int index, E elm) {
			...
			if (index == size())
				...
				newNode = new Node(elm, prevNode.getNext(), prevNode);
		...
		public boolean remove(int index) {
			...
			Node curNode = previousNode.getNext();
			Node nextNode = curNode.getNext();
			...
			for (Node node = header; node != trailer; node = node.getNext())
			{
				if (curNode.equals(rmNode))
				{
